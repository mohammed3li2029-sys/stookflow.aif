/* ===================================================================
   ...
   assignment is automatically mirrored to Firestore (when configured).
   In demo mode (no Firebase config) this is a harmless no-op and the
...
function withFirestoreSync(arr, collectionName, idField){
  return new Proxy(arr, {
    set(target, prop, value){
      target[prop] = value;
      if(window.StockFlowFirebase && window.StockFlowFirebase.enabled && prop !== 'length'){
        window.StockFlowFirebase.syncCollection(collectionName, target, idField);
      }
      return true;
    },
    deleteProperty(target, prop){
      delete target[prop];
      if(window.StockFlowFirebase && window.StockFlowFirebase.enabled){
        window.StockFlowFirebase.syncCollection(collectionName, target, idField);
      }
      return true;
    }
...
/* Once Firebase reports its status, try loading Inventory & Warehouses
   from Firestore. If the collections are empty/unavailable, the
   built-in demo data (already on screen) is left untouched — so the
   app always works, with or without Firebase configured. */
window.addEventListener('stockflow-firebase-ready', async ()=>{
  if(!window.StockFlowFirebase || !window.StockFlowFirebase.enabled) return;
  try{
    const inv = await window.StockFlowFirebase.loadCollection('inventory');
    if(inv && inv.length){
      inventoryData.length = 0;
      inv.forEach(item => inventoryData.push(item));
      if(typeof renderInventory === 'function') renderInventory();
    }
    const wh = await window.StockFlowFirebase.loadCollection('warehouses');
    if(wh && wh.length){
    ...
    console.error('[StockFlow] Firestore initial load failed:', err);
  }
});
...
function doLogout(){
  if(window.StockFlowFirebase && window.StockFlowFirebase.enabled){
    window.StockFlowFirebase.signOutUser().catch(()=>{});
  }
...
function doLogin(){
  ...
  // If Firebase Auth is configured and the entered username looks like an
  // email, try real Firebase sign-in first. Otherwise (or on failure),
  // fall back to the built-in demo credentials so the app still works
  // out of the box with no Firebase project set up.
  const tryFirebase = window.StockFlowFirebase && window.StockFlowFirebase.enabled && uname.includes('@');
  const firebaseAttempt = tryFirebase
    ? window.StockFlowFirebase.signInWithEmail(uname, upass)
        .then(cred => ({
          username: uname,
